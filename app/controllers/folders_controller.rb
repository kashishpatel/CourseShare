class FoldersController < ApplicationController
  ...
  def new
   ...
   if params[:folder_id] #if we want to create a folder inside another folder 
     @folder.course = current_user.folders.find(params[:folder_id]).course
    ...
    respond_to do |format|
      if @folder.update(folder_params)
        if @folder.parent == nil
          ...
          format.html { redirect_to browse_path(@folder.parent), notice: 'Folder was successfully updated.' }
        end
  ...
  private
    ...
    def folder_params
      params.require(:folder).permit(:name, :parent_id, :user_id, :folder, :folder_id, :attachment, :course)
    end
end
